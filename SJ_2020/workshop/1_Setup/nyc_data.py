...
def download_nyctaxi_data(years, path):
    ...
    if not set(years) <= set(taxi_years):
        print(f' {years} years list not valid! '.center(80, '#'))
        print(f' Please specify a sublist of {taxi_years} '.center(80, '#'))
        raise  Exception("{years} years list is not valid".format(years=years))
